...
class BST {
  private:
    ...
    void insert(Node<T>* &p, Node<T> *newNode) {
        ...
        // cout << "entered insert func: p =" << p << " | newNode = " << newNode << endl;
        // if (p == newNode) {
        //     p->count += newNode->count;
        // }
        if (p == nullptr) {
            p = newNode;
            this->nodeCount++;
    ...
    Node<T>* findMax(Node<T>* p) {
        if (p->rTree == nullptr) return p;
        return findMax(p->rTree);
    }
